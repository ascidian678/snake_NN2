    ...
    def play(self, testNN=False, _model=None):
        ...
        while 1:
            ...
            if testNN == False:
                ...
                # print(front_blocked, left_blocked, right_blocked, apple_dir[0], apple_dir[1],
                #                      snake_dir[0], snake_dir[1])
                # print(new_direction)
            else:
                # snake_vect_norm_ = np.linalg.norm(snake_dir)
                # apple_vect_norm_ = np.linalg.norm(apple_dir)
                input_test_vector = [left_blocked, front_blocked, right_blocked, apple_dir[1], apple_dir[0],
                                     snake_dir[1], snake_dir[0]]

                input_test_vector = np.array(input_test_vector).reshape(-1, 7)  # creating a vector from a list KERAS
                print("making decision")
                print(left_blocked, front_blocked, right_blocked, apple_dir[1], apple_dir[0],
                                     snake_dir[1], snake_dir[0])
                ...
                print('argmaxOut:')
                print(predicted_direction_index)
                # if predicted_direction.tolist() == [1, 0, 0]:
                #     new_direction = left_direction_vector
                # if predicted_direction.tolist() == [0, 0, 1]:
                #     new_direction = right_direction_vector
            ...
            if (snake[0][0] in [box[0][0], box[1][0]] or
                ...
                # logger.info('*')
                # logger.info('blocked dirs:')
                # logger.info(str((left_blocked, front_blocked, right_blocked)))
                # logger.info('dir before correction:')
                ...
                # logger.info(str((left_blocked, front_blocked, right_blocked, apple_dir[0], apple_dir[1],
                #                    snake_dir[0], snake_dir[1])))
                # logger.info(str((direction)))
                ...
                output_vect.append(direction)
                # logger.info('*')
                # logger.info(str((front_blocked, left_blocked, right_blocked, apple_dir[0], apple_dir[1],
                #                    snake_dir[0], snake_dir[1])))
                # logger.info(str((direction)))
